...
GegeGraph::GegeGraph(EdgeList src_sorted_edges, EdgeList dst_sorted_edges, int64_t num_nodes_in_memory) {
    ...
    all_dst_sorted_edges_ = dst_sorted_edges_;
...
Indices GegeGraph::getNodeIDs() { return node_ids_; }

Indices GegeGraph::getEdges(bool incoming) {
    if (incoming) {
...
Indices GegeGraph::getRelationIDs(bool incoming) {
...
Indices GegeGraph::getNeighborOffsets(bool incoming) {
    if (incoming) {
        return in_offsets_;
    } else {
        return out_offsets_;
...
Indices GegeGraph::getNumNeighbors(bool incoming) {
    if (incoming) {
...
void GegeGraph::to(torch::Device device) {
...
// 1 hop sampler
std::tuple<torch::Tensor, torch::Tensor> GegeGraph::getNeighborsForNodeIds(torch::Tensor node_ids, bool incoming,
