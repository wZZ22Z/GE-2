...
LayeredNeighborSampler::LayeredNeighborSampler(shared_ptr<GegeGraph> graph, std::vector<shared_ptr<NeighborSamplingConfig>> layer_configs) {
    graph_ = graph;
    storage_ = nullptr;
    sampling_layers_ = layer_configs;
}
...
DENSEGraph LayeredNeighborSampler::getNeighbors(torch::Tensor node_ids, shared_ptr<GegeGraph> graph) {
    Indices hop_offsets;
    ...
    if (graph == nullptr) {
        if (storage_ != nullptr) {
            ...
            throw GegeRuntimeException("Graph to sample from is undefined");
        }
    }
