...
StorageBackend getStorageBackend(std::string string_val) {
    ...
    } else if (string_val == "MEM_PARTITION_BUFFER") {
        return StorageBackend::MEM_PARTITION_BUFFER;
    } else {
...
NegativeSamplingMethod getNegativeSamplingMethod(std::string string_val) {
    for (auto& c : string_val) c = toupper(c);

    if (string_val == "RNS") {
        return NegativeSamplingMethod::RNS;
    } else if (string_val == "DNS") {
        return NegativeSamplingMethod::DNS;
    } else if (string_val == "GAN") {
        return NegativeSamplingMethod::GAN;
    } else if (string_val == "OTHER") {
        return NegativeSamplingMethod::OTHER;
    } else {
        throw std::runtime_error("Unrecognized negative sampling method string");
    }
}
