...
shared_ptr<StorageBackendConfig> initStorageBackendConfig(pyobj python_config) {
    ...
    if (ret_config->type == StorageBackend::PARTITION_BUFFER || ret_config->type == StorageBackend::MEM_PARTITION_BUFFER) {
...
shared_ptr<GegeConfig> initGegeConfig(pyobj python_config) {
    shared_ptr<GegeConfig> ret_config = std::make_shared<GegeConfig>();
...
shared_ptr<GegeConfig> loadConfig(string config_path, bool save) {
    string module_name = "gege.tools.configuration.gege_config";
    shared_ptr<GegeConfig> ret;
    if (Py_IsInitialized() != 0) {
        ...
        ret = initGegeConfig(python_config);
    } else {
        setenv("GEGE_NO_BINDINGS", "1", true);
