...
void LinkPredictionReporter::save(string directory, bool scores, bool ranks) {
    ...
    if (ranks || scores) {
        if (per_batch_edges_.empty()) {
            throw GegeRuntimeException("To save scores or ranks, the evaluated edges must be provided to addResult()");
        }
...
void NodeClassificationReporter::save(string directory, bool labels) {
    ...
    if (labels) {
        if (per_batch_nodes_.empty()) {
            throw GegeRuntimeException("To save labels, the evaluated node ids must be provided to add_result()");
        }
