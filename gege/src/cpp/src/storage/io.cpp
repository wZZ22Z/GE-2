...
std::tuple<shared_ptr<Storage>, shared_ptr<Storage>, shared_ptr<Storage>> initializeEdges(shared_ptr<StorageConfig> storage_config,
    ...
    if (storage_config->embeddings != nullptr) {
        ...
        if (storage_config->embeddings->type == StorageBackend::MEM_PARTITION_BUFFER) {
            use_buffer = true;
        }
    }
...
std::tuple<shared_ptr<Storage>, shared_ptr<Storage>, shared_ptr<Storage>, shared_ptr<Storage>> initializeNodeEmbeddings(shared_ptr<Model> model, shared_ptr<StorageConfig> storage_config,
                                                                              bool reinitialize, bool train, NegativeSamplingMethod nsm, shared_ptr<InitConfig> init_config) {
    ...
    string node_embedding_g_filename;
    string optimizer_state_g_filename;
    if (nsm == NegativeSamplingMethod::GAN) {
        node_embedding_g_filename = storage_config->model_dir + PathConstants::embeddings_g_file + PathConstants::file_ext;
        optimizer_state_g_filename = storage_config->model_dir + PathConstants::embeddings_g_state_file + PathConstants::file_ext;
    }

    if (storage_config->embeddings == nullptr || !model->has_embeddings()) {
        return std::forward_as_tuple(nullptr, nullptr, nullptr, nullptr);
    }
    ...
    if (reinitialize) {
        ...
        shared_ptr<FlatFile> init_node_embeddings_g;
        shared_ptr<FlatFile> init_optimizer_state_storage_g;
        if (nsm == NegativeSamplingMethod::GAN) {
            init_node_embeddings_g = std::make_shared<FlatFile>(node_embedding_g_filename, dtype);
            init_optimizer_state_storage_g = std::make_shared<FlatFile>(optimizer_state_g_filename, dtype);
        }
        ...
        while (offset < num_nodes) {
            ...
            if (nsm == NegativeSamplingMethod::GAN) {
                weights2 = initialize_subtensor(init_config, {curr_num_nodes, embedding_dim}, {num_nodes, embedding_dim}, torch::TensorOptions());
                init_node_embeddings_g->append(weights2);
                init_optimizer_state_storage_g->append(emb_state);
            }
    ...
    shared_ptr<Storage> optimizer_state_storage_g = nullptr;

    std::vector<torch::Device> devices = devices_from_config(storage_config);
    ...
    switch (storage_config->embeddings->type) {
        ...
        case StorageBackend::PARTITION_BUFFER: {
            ...
            if (nsm == NegativeSamplingMethod::GAN) {
                SPDLOG_ERROR("GAN not available for PARTITION_BUFFER.");
                throw std::runtime_error("");
            }
            if (train) {
        ...
        case StorageBackend::HOST_MEMORY: {
            ...
            if (nsm == NegativeSamplingMethod::GAN) {
                node_embeddings_g = std::make_shared<InMemory>(node_embedding_g_filename, num_nodes, embedding_dim, dtype, torch::kCPU);
            }
            if (train) {
                optimizer_state_storage = std::make_shared<InMemory>(optimizer_state_filename, num_nodes, embedding_dim, dtype, torch::kCPU);
                if (nsm == NegativeSamplingMethod::GAN) {
                    optimizer_state_storage_g = std::make_shared<InMemory>(optimizer_state_g_filename, num_nodes, embedding_dim, dtype, torch::kCPU);
                }
            }
            break;
        }
        case StorageBackend::DEVICE_MEMORY: {
            ...
            if (nsm == NegativeSamplingMethod::GAN) {
                node_embeddings_g = std::make_shared<InMemory>(node_embedding_g_filename, num_nodes, embedding_dim, dtype, storage_config->device_type);
            }
            if (train) {
                ...
                if (nsm == NegativeSamplingMethod::GAN) {
                    optimizer_state_storage_g = std::make_shared<InMemory>(optimizer_state_g_filename, num_nodes, embedding_dim, dtype, storage_config->device_type);
                }
            }
            break;
        }
...
shared_ptr<GraphModelStorage> initializeStorageLinkPrediction(shared_ptr<Model> model, shared_ptr<StorageConfig> storage_config, bool reinitialize, bool train,
    ...
    if (nsm == NegativeSamplingMethod::GAN) {
        storage_ptrs.node_embeddings_g = std::get<2>(node_embeddings);
        storage_ptrs.node_optimizer_state_g = std::get<3>(node_embeddings);
    }
...
shared_ptr<GraphModelStorage> initializeStorageNodeClassification(shared_ptr<Model> model, shared_ptr<StorageConfig> storage_config, bool reinitialize,
    ...
    std::tuple<shared_ptr<Storage>, shared_ptr<Storage>, shared_ptr<Storage>, shared_ptr<Storage>> node_embeddings =
            initializeNodeEmbeddings(model, storage_config, reinitialize, train, NegativeSamplingMethod::OTHER, init_config);
    storage_ptrs.node_embeddings = std::get<0>(node_embeddings);
    storage_ptrs.node_optimizer_state = std::get<1>(node_embeddings);
...
shared_ptr<GraphModelStorage> initializeStorage(shared_ptr<Model> model, shared_ptr<StorageConfig> storage_config, bool reinitialize, bool train,
    ...
    if (model->learning_task_ == LearningTask::LINK_PREDICTION) {
        return initializeStorageLinkPrediction(model, storage_config, reinitialize, train, nsm, init_config);
