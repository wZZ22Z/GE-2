...
void init_options(py::module &m) {
    ...
    py::enum_<StorageBackend>(m, "StorageBackend")
        ...
        .value("MEM_PARTITION_BUFFER", StorageBackend::MEM_PARTITION_BUFFER);
    ...
    py::enum_<NegativeSamplingMethod>(m, "NegativeSamplingMethod")
        .value("RNS", NegativeSamplingMethod::RNS)
        .value("DNS", NegativeSamplingMethod::DNS)
        .value("GAN", NegativeSamplingMethod::GAN)
        .value("OTHER", NegativeSamplingMethod::OTHER);

    m.def("getNegativeSamplingMethod", &getNegativeSamplingMethod, py::arg("string_val"));
