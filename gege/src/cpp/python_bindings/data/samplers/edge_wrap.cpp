...
class PyEdgeSampler : EdgeSampler {
   public:
    ...
    EdgeList getEdges(shared_ptr<Batch> batch, int32_t device_idx) override { PYBIND11_OVERRIDE_PURE_NAME(EdgeList, EdgeSampler, "getEdges", getEdges, batch, device_idx); }
};
...
void init_edge_samplers(py::module &m) {
    py::class_<EdgeSampler, PyEdgeSampler, std::shared_ptr<EdgeSampler>>(m, "EdgeSampler")
        ...
        .def("getEdges", &EdgeSampler::getEdges, py::arg("batch"), py::arg("device_idx"));
