...
void init_dataloader(py::module &m) {
    py::class_<DataLoader, shared_ptr<DataLoader>>(m, "DataLoader", py::dynamic_attr())
        ...
        .def_readwrite("all_batches", &DataLoader::all_batches_)
        ...
        .def_readwrite("all_read", &DataLoader::all_reads_)
        ...
        .def(py::init([](shared_ptr<GraphModelStorage> graph_storage, std::string learning_task, shared_ptr<TrainingConfig> training_config,
                 ...
                 return std::make_shared<DataLoader>(graph_storage, task, training_config, evaluation_config, encoder_config, devices);
             }),
             py::arg("graph_storage"), py::arg("learning_task"), py::arg("training_config"), py::arg("evaluation_config"), py::arg("encoder_config"), py::arg("devices"))
        ...
        .def(py::init([](shared_ptr<GraphModelStorage> graph_storage, std::string learning_task, int batch_size, shared_ptr<NegativeSampler> neg_sampler,
                 ...
                 if (task == LearningTask::LINK_PREDICTION) {
                     ...
                     if (graph_storage->storage_ptrs_.nodes == nullptr) {
                         throw GegeRuntimeException("Node ids must be provided for node classification");
                     }

                     if (train) {
                         if (graph_storage->storage_ptrs_.node_labels == nullptr) {
                             throw GegeRuntimeException("Labels for the nodes must be provided when training with node classification");
                         }
        ...
        .def(py::init([](torch::optional<torch::Tensor> edges, std::string learning_task, torch::optional<torch::Tensor> nodes,
                 ...
                 if (nodes.has_value()) {
                     ...
                     if (task == LearningTask::NODE_CLASSIFICATION) {
                         throw GegeRuntimeException("Tensor of node ids must be provided for node classification");
                     }
                 }
        ...
        .def("initializeBatches", &DataLoader::initializeBatches, py::arg("prepare_encode") = false, py::arg("device_idx") = 0)
        ...
        .def("getBatch", &DataLoader::getBatch, py::arg("device") = py::none(), py::arg("perform_map") = true, py::arg("device_idx") = 0, py::return_value_policy::reference)
        .def("edgeSample", &DataLoader::edgeSample, py::arg("batch"), py::arg("device_idx"))
        .def("nodeSample", &DataLoader::nodeSample, py::arg("batch"), py::arg("device_idx"))
        .def("loadCPUParameters", &DataLoader::loadCPUParameters, py::arg("batch"))
        .def("loadGPUParameters", &DataLoader::loadGPUParameters, py::arg("batch"), py::arg("device_idx"))
        .def("updateEmbeddings", &DataLoader::updateEmbeddings, py::arg("batch"), py::arg("gpu") = false, py::arg("device_idx") = 0)
