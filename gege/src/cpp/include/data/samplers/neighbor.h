...
class NeighborSampler {
   public:
     ...
     */
    virtual DENSEGraph getNeighbors(torch::Tensor node_ids, shared_ptr<GegeGraph> graph = nullptr) = 0;
};
...
class LayeredNeighborSampler : public NeighborSampler {
   public:
    ...
    LayeredNeighborSampler(shared_ptr<GegeGraph> graph, std::vector<shared_ptr<NeighborSamplingConfig>> layer_configs);

    LayeredNeighborSampler(std::vector<shared_ptr<NeighborSamplingConfig>> layer_configs);

    DENSEGraph getNeighbors(torch::Tensor node_ids, shared_ptr<GegeGraph> graph = nullptr) override;
