...
from gege.tools.configuration.constants import PathConstants
from gege.tools.configuration.datatypes import (
    AdagradOptions,
    AdamOptions,
    ConstantInitOptions,
    DecoderOptions,
...
from gege.tools.configuration.validation import (
...
class NeighborSamplingConfig:
    type: str = "ALL"
    options: NeighborSamplingOptions = field(default_factory=NeighborSamplingOptions)
...
class OptimizerConfig:
    type: str = "ADAGRAD"
    options: OptimizerOptions = field(default_factory=AdagradOptions)
...
class LossConfig:
    type: str = "SOFTMAX_CE"
    options: LossOptions = field(default_factory=LossOptions)
...
class LayerConfig:
    ...
    input_dim: int = -1
    output_dim: int = -1
    init: InitConfig = field(default_factory=lambda: InitConfig(type="GLOROT_UNIFORM"))
    optimizer: OptimizerConfig = field(default_factory=lambda: OptimizerConfig(type="DEFAULT"))
    bias: bool = False
    bias_init: InitConfig = field(default_factory=lambda: InitConfig(type="ZEROS"))
    activation: str = "NONE"
...
class ModelConfig:
    ...
    dense_optimizer: OptimizerConfig = field(default_factory=OptimizerConfig)
    sparse_optimizer: OptimizerConfig = field(default_factory=OptimizerConfig)

    def __post_init__(self):
        if self.random_seed is MISSING:
            # self.random_seed = 6949716878175764926
...
class StorageBackendConfig:
    ...
    options: StorageOptions = field(default_factory=lambda: StorageOptions(dtype="float"))
    ...
    def merge(self, input_config: DictConfig):
        ...
        if self.type == "PARTITION_BUFFER" or self.type == "MEM_PARTITION_BUFFER":
...
class DatasetConfig:
    ...
    def populate_dataset_stats(self):
        ...
        if not dataset_stats_path.exists():
            raise ValueError(
                "{} does not exist, expected to see dataset.yaml file in {} generated by gege_preprocess".format(
...
class StorageConfig:
    ...
    edges: StorageBackendConfig = field(
        default_factory=lambda: StorageBackendConfig(options=StorageOptions(dtype="int"))
    )
    nodes: StorageBackendConfig = field(
        default_factory=lambda: StorageBackendConfig(options=StorageOptions(dtype="int"))
    )
    embeddings: StorageBackendConfig = field(
        default_factory=lambda: StorageBackendConfig(options=StorageOptions(dtype="float"))
    )
    features: StorageBackendConfig = field(
        default_factory=lambda: StorageBackendConfig(options=StorageOptions(dtype="float"))
    )
    prefetch: bool = True
    ...
    SUPPORTED_EMBEDDING_BACKENDS = ["PARTITION_BUFFER", "DEVICE_MEMORY", "HOST_MEMORY", "MEM_PARTITION_BUFFER"]
...
def type_safe_merge(base_config: GegeConfig, input_config: DictConfig):
    """
    Merges under specified dictionary config into the current configuration object
...
def load_config(input_config_path, save=False):
    ...
    if save and (output_config.training.resume_from_checkpoint != "" or not output_config.training.resume_training):
        ...
        # could also be taken when gege_predict is run or gege_train is run with resume_training set to true,
